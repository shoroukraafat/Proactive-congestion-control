...
void MoveCSpeedUesToNormalPosition(NodeContainer CSpeedUes, uint32_t nCSpeedUes, Ptr<LteHelper> lteHelper, Ptr<ListPositionAllocator> CSpeedUesPositionAlloc) {
...
int main(int argc, char* argv[])
{
    ...
    uint32_t nCSpeedUes = 10;// Number of users with constant speed mobility model
    ...
    for (uint16_t i = 0; i < nCSpeedUes / 2; i++) {
        Vector enbPosition(3000 * i, 5000, 2);
        CSpeedUesPositionAlloc->Add(enbPosition);
    }
    for (uint16_t i = 0; i < nCSpeedUes / 2; i++) {
        Vector enbPosition(3000 * i, 7000, 2);
        CSpeedUesPositionAlloc->Add(enbPosition);
    }
    MobilityHelper CSpeedUesMobility;
    CSpeedUesMobility.SetMobilityModel("ns3::ConstantVelocityMobilityModel");
    ...
    Simulator::Schedule(Seconds(30.0), &MoveCSpeedUesToNormalPosition, CSpeedUes, nCSpeedUes, lteHelper, CSpeedUesPositionAlloc);
